export default async function handler(req, res) {
  ...
  try {
    ...
    if (insertData.code !== 0) {
      console.error("Bitable insert error:", insertData);
      return res.status(500).json({ error: 'Failed to insert to Bitable' });
    }

    res.status(200).json({ success: true });
  } catch (error) {
    console.error("Bitable integration error:", error);
